...
import { SafeAreaView, StatusBar, StyleSheet, Text, TouchableWithoutFeedback, View, AsyncStorage } from "react-native";
...
const { STORE_EMAIL_KEY_CURRENT, STORE_EMAIL_KEY } = Constant
...
export default class SignUpA extends Component {
  ...
  setData = async (emailKey) => {
    try {
      await AsyncStorage.setItem(emailKey, JSON.stringify({ }))        
    } catch (e) {
      // saving error
      console.error(' asyncStorage.setItem error : ', emailKey)
    }
  }

  storeData = async (storeKey) => {
    const emailKey = STORE_EMAIL_KEY + storeKey
    try {
      const value = await AsyncStorage.getItem(emailKey)
      if(value == null) {
        this.setData(emailKey)
      }
      await AsyncStorage.setItem(STORE_EMAIL_KEY_CURRENT, emailKey) 
    } catch(e) {
      // error reading value
      console.error(' asyncStorage.getItem error : ', emailKey, e)
    }
  }
